...
namespace CleanWpfApp
{
    ...
    internal sealed class Journal : ISerializable
    {
        ...
        internal JournalEntry? RemoveBackEntry()
        {
            ...
            var index = _currentEntryIndex; // start from current but do not change it
            do
            {
                if (--index < 0)
                {
            ...
            var removedEntry = RemoveEntryInternal(index);
            Debug.Assert(ValidateIndexes());
            UpdateView();
            return removedEntry;
        }
        ...
        internal bool ClearForwardStack()
        {
            ...
            {
                throw new InvalidOperationException(Strings.InvalidOperation_CannotClearFwdStack);
            }
        ...
        internal JournalEntry? BeginBackNavigation()
        {
            ...
            JournalEntry journalEntry = GetGoBackEntry(out var index) ?? throw new InvalidOperationException(Strings.NoBackEntry);
