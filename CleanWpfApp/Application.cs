...
namespace CleanWpfApp
{
    ...
    public class Application : DispatcherObject, IHaveResources, IQueryAmbient
    {
        ...
        public static void LoadComponent(object component, Uri resourceLocator)
        {
            ...
            {
                throw new ArgumentException(SR.Format(Strings.ArgumentPropertyMustNotBeNull, "resourceLocator", "OriginalString"));
            }
            ...
            var currentUri = new Uri(BaseUriHelper.PackAppBaseUri, resourceLocator);
            ...
            var closeStream = true;  // Whether or not to close the stream after LoadBaml is done.
            ...
            else
            {
                ...
                var contentType = new ContentType(part.ContentType);
                ...
                {
                    throw new Exception(SR.Format(Strings.ContentTypeNotSupported, contentType));
                }
            }


            if (stream is not IStreamInfo bamlStream || bamlStream.Assembly != component.GetType().Assembly)
            {
                throw new Exception(SR.Format(Strings.UriNotMatchWithRootType, component.GetType(), resourceLocator));
            }

            XamlReader.LoadBaml(stream, pc, component, closeStream);
        }
        ...
        /// </summary>
        /// <param name="resourceLocator">Resource Locator</param>
        /// <param name="skipJournaledProperties">SkipJournaledProperties or not</param>
        internal static object LoadComponent(Uri resourceLocator, bool skipJournaledProperties)
        {
            //
            ...
            //..\..\ in the relative Uri will get stripped when creating the new Uri and resolving to the
            //PackAppBaseUri, i.e. only relative Uri within the appbase are created here
            var packUri = BindUriHelper.GetResolvedUri(BaseUriHelper.PackAppBaseUri, resourceLocator);

            var part = GetResourceOrContentPart(packUri);
            var contentType = new ContentType(part.ContentType);
            var stream = part.GetSeekableStream();

            var pc = new ParserContext
            {
                BaseUri = packUri,
                SkipJournaledProperties = skipJournaledProperties
            };
            ...
            {
                throw new Exception(SR.Format(Strings.ContentTypeNotSupported, contentType.ToString()));
            }
        }
        ...
        internal static object LoadBamlStreamWithSyncInfo(Stream stream, ParserContext pc)
        {
            ...
            //
            var loadBamlSyncInfo = new NestedBamlLoadInfo(pc.BaseUri, stream, pc.SkipJournaledProperties);
        ...
        public static StreamResourceInfo GetResourceStream(Uri uriResource)
        {
            ...
            if (uriResource.OriginalString == null)
                throw new ArgumentException(SR.Format(Strings.ArgumentPropertyMustNotBeNull, "uriResource", "OriginalString"));
