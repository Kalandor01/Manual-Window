...
namespace CleanWpfApp
{
    ...
    internal class ResourceContainer : Package
    {
        ...
        internal static ResourceManagerWrapper ApplicationResourceManagerWrapper
        {
            get
            {
                if (_applicationResourceManagerWrapper == null)
                {
                    ...
                    var asmApplication = Application.ResourceAssembly;
        ...
        protected override PackagePart GetPartCore(Uri uri)
        {
            ...
            var rmWrapper = GetResourceManagerWrapper(uri, out var partName, out var isContentFile);
        ...
        private void OnAssemblyLoadEventHandler(object sender, AssemblyLoadEventArgs args)
        {
            ...
            {
                var assemblyInfo = new AssemblyName(assembly.FullName);

                var assemblyName = assemblyInfo.Name.ToLowerInvariant();
                var assemblyKey = string.Empty;
               ...
               var reqKeyToken = assemblyInfo.GetPublicKeyToken();
        ...
        {
            if (_registeredResourceManagers.TryGetValue(key, out var value))
            {
        ...
        {
            var rmwResult = ApplicationResourceManagerWrapper;
            ...
            {
                var key = assemblyName + assemblyVersion + assemblyKey;
            ...
            if (rmwResult == ApplicationResourceManagerWrapper)
            {
                ...
                else
                {
                    ...
                    throw new IOException(Strings.EntryAssemblyIsNull);
                }
            }
        ...
        private static Dictionary<string, ResourceManagerWrapper> _registeredResourceManagers = new();
