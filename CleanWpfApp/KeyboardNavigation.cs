...
namespace CleanWpfApp
{
    ...
    public sealed class KeyboardNavigation
    {
        ...
        {
            var weakRef = (WeakReference)d.GetValue(TabOnceActiveElementProperty);
            if (weakRef != null && weakRef.IsAlive)
            {
                var activeElement = weakRef.Target as DependencyObject;
        ...
        {
            var weakRef = (WeakReference)d.GetValue(ControlTabOnceActiveElementProperty);
            if (weakRef != null && weakRef.IsAlive)
            {
                var activeElement = weakRef.Target as DependencyObject;
                // Verify if the element is still in the same visual tree
        ...
        private WeakReferenceList _weakFocusChangedHandlers = new();
        #endregion
        #endregion
        ...
        /// <seealso cref="IsTabStopProperty" />
        ...
        /// <param name="element">The element to which to write the attached property.</param>
        /// <param name="mode">The property value to set</param>
        /// <seealso cref="ControlTabNavigationProperty" />
        ...
        private sealed class FocusVisualAdorner : Adorner
        {
            ...
            public FocusVisualAdorner(
                ContentElement adornedElement,
                UIElement adornedElementParent,
                IContentHost contentHostParent,
                Style focusVisualStyle
            )
            ...
            protected override Size ArrangeOverride(Size size)
            {
                ...
                if (_adornedContentElement != null)
                {
                    ...
                    else
                    {
                        ...
                        if (contentHost is not Visual visualContent || !AdornedElement.IsAncestorOf(visualContent))
                        {
                        ...
                        var enumerator = _contentRects.GetEnumerator();
            ...
            protected override Visual GetVisualChild(int index)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException(nameof(index), index, Strings.Visual_ArgumentOutOfRange);
                }
            }
            ...
            internal override bool NeedsUpdate(Size oldSize)
            {
                if (_adornedContentElement != null)
                {
                    ...
                    if (contentHost is Visual visualContent && AdornedElement.IsAncestorOf(visualContent))
                    {
                        _hostToAdornedElement = visualContent.TransformToAncestor(AdornedElement);
                    }
                    else
                    {
                    ...
                    if (oldTransform != _hostToAdornedElement)
                    {
                        ...
                        if (
                            oldTransform is not MatrixTransform oldMatrix ||
                            _hostToAdornedElement is not MatrixTransform matrix ||
                            !Matrix.Equals(oldMatrix.Matrix, matrix.Matrix)
                        )
                        {
        ...
        internal static UIElement? GetParentUIElementFromContentElement(ContentElement ce, ref IContentHost ichParent)
        {
            ...
            if (ich is DependencyObject parent)
            {
                // Case 1: UIElement
                // return the element
                if (parent is UIElement eParent)
                {
                    return eParent;
                }
                ...
                while (visualParent != null)
                {
                    ...
                    if (visualParent is UIElement uielement)
                    {
                        return uielement;
                    }
                }
                ...
                if (parent is ContentElement ceParent)
                {
        ...
        internal static void UpdateFocusedElement(DependencyObject focusTarget)
        {
            ...
            if (focusScope != null && focusScope != focusTarget)
            {
                ...
                var visualRoot = GetVisualRoot(focusTarget);
        ...
        private void UpdateActiveElement(DependencyObject activeElement, DependencyProperty dp)
        {
            ...
            var container = GetGroupParent(activeElement);
        ...
        )
        {
            ...
            if (nextTab == null)
            {
                ...
                var navigatedOutside = NavigateOutsidePresentationSource(currentElement, request, fromProcessInputTabKey, ref shouldCycle);
                if (navigatedOutside)
                {
                    return true;
                }
            ...
            if (nextTab is not IKeyboardInputSink inputSink)
            {
                ...
                var iie = nextTab as IInputElement;
                iie.Focus();
            ...
            else
            {
                ...
                {
                    var tr = new TraversalRequest(request.FocusNavigationDirection)
                    {
                        Wrapped = true
                    };
        ...
        {
            if (currentElement is not Visual visual)
            {
            ...
            if (PresentationSource.CriticalFromVisual(visual) is IKeyboardInputSink inputSink)
            {
                var ikis = inputSink.KeyboardInputSite;
                ...
                if (ikis != null && ShouldNavigateOutsidePresentationSource(currentElement, request))
                {
                    ...
                    {
                        if (ikis is System.Windows.Input.IAvalonAdapter avalonAdapter && fromProcessInput)
                        {
        ...
        private bool ShouldNavigateOutsidePresentationSource(DependencyObject currentElement, TraversalRequest request)
        {
            ...
            if (
                request.FocusNavigationDirection == FocusNavigationDirection.Left ||
        ...
        private void ProcessInput(InputEventArgs inputEventArgs)
        {
            ...
            var keyEventArgs = (KeyEventArgs)inputEventArgs;
            if (keyEventArgs.Handled)
            {
                return;
            }

            var sourceElement = keyEventArgs.OriginalSource as DependencyObject;
            ...
            if (keyEventArgs.KeyboardDevice.Target is DependencyObject innerElement && sourceElement != innerElement)
            {
            ...
            {
                if (keyEventArgs.UnsafeInputSource is not HwndSource hwndSource)
                {
                    return;
                }
        ...
        internal static void EnableKeyboardCues(DependencyObject element, bool enable)
        {
            ...
            var rootVisual = GetVisualRoot(visual);
        ...
        internal DependencyObject? PredictFocusedElement(
        ...
        )
        {
            ...
            var result = FindElementAtViewportEdge(
                sourceElement,
                viewportBoundsElement,
                container,
                direction,
        ...
        internal bool Navigate(DependencyObject sourceElement, Key key, ModifierKeys modifiers, bool fromProcessInput = false)
        {
            return key switch
            {
                ...
                Key.Tab => Navigate(sourceElement,
                ...
                Key.Right => Navigate(sourceElement, new TraversalRequest(FocusNavigationDirection.Right), modifiers),
                Key.Left => Navigate(sourceElement, new TraversalRequest(FocusNavigationDirection.Left), modifiers),
                Key.Up => Navigate(sourceElement, new TraversalRequest(FocusNavigationDirection.Up), modifiers),
                Key.Down => Navigate(sourceElement, new TraversalRequest(FocusNavigationDirection.Down), modifiers),
                _ => false,
            };
        }
        #endregion
        ...
        {
            if (visual is UIElement uiElement && uiElement.IsVisible)
            {
                return true;
            }

            if (visual is IContentHost && visual is not Documents.UIElementIsland)
            {
                return true;
            }

            if (visual is UIElement3D uiElement3D && uiElement3D.IsVisible)
            {
                return true;
            }
        ...
        private DependencyObject? GetPreviousSibling(DependencyObject e)
        {
            ...
            if (parent is IContentHost ich)
            {
                IInputElement? previousElement = null;
                var enumerator = ich.HostedElements;
                while (enumerator.MoveNext())
                {
                    ...
                    {
                        if (current is ContentElement ce && IsTabStop(ce))
                        {
                            previousElement = current;
                        }
                    }
        ...
        private DependencyObject? GetNextSibling(DependencyObject e)
        {
            ...
            else
            {
                ...
                if (parentAsUIElement != null && elementAsVisual != null)
                {
                    ...
                    for (; i < count; i++)
                    {
                        var visual = VisualTreeHelper.GetChild(parentAsUIElement, i);
        ...
        private DependencyObject? FocusedElement(DependencyObject e)
        {
            ...
            if (e is IInputElement iie && !iie.IsKeyboardFocusWithin)
            {
                if (FocusManager.GetFocusedElement(e) is DependencyObject focusedElement)
                {
                    if (_navigationProperty == ControlTabNavigationProperty || !IsFocusScope(e))
                    {
                        // Verify if focusedElement is a visual descendant of e
                        var visualFocusedElement = focusedElement as Visual;
                        if (visualFocusedElement == null)
                        {
                            if (focusedElement is not Visual3D visual3DFocusedElement)
                            {
        ...
        private DependencyObject? GetFirstChild(DependencyObject e)
        {
            ...
            var focusedElement = FocusedElement(e);
        ...
        private DependencyObject? GetLastChild(DependencyObject e)
        {
            ...
            if (uiElement == null || UIElementHelper.IsVisible(uiElement))
            {
                ...
                if (elementAsVisual != null)
                {
                    ...
                    {
                        var visual = VisualTreeHelper.GetChild(elementAsVisual, i);
                        if (IsInNavigationTree(visual))
                        {
                            return visual;
                        }
                        ...
                        {
                            var lastChild = GetLastChild(visual);
                            if (lastChild != null)
                            {
                                return lastChild;
                            }
        ...
        internal static DependencyObject? GetParent(DependencyObject e)
        {
            ...
            else
            {
                ...
                if (e is ContentElement contentElement)
                {
        ...
        *        3   2
        ...
        private DependencyObject GetGroupParent(DependencyObject e, bool includeCurrent)
        {
            ...
            var result = e; // Keep the last non null element
        ...
        private bool IsTabStop(DependencyObject e)
        {
            ...
            {
                return fe.Focusable && (bool)fe.GetValue(IsTabStopProperty) && fe.IsEnabled && fe.IsVisible;
            }

            return e is FrameworkContentElement fce && fce.Focusable && (bool)fce.GetValue(IsTabStopProperty) && fce.IsEnabled;
        }
        ...
        internal DependencyObject? GetFirstTabInGroup(DependencyObject container)
        {
            ...
            var minIndexFirstTab = int.MinValue;
        ...
        internal DependencyObject? GetLastTabInGroup(DependencyObject container)
        {
            ...
            var maxIndexFirstTab = int.MaxValue;
        ...
        internal static Rect GetRectangle(DependencyObject element)
        {
            if (element is UIElement uiElement)
            {
                ...
                var rootVisual = GetVisualRoot(uiElement);

                if (rootVisual != null)
                {
                    var transform = uiElement.TransformToAncestor(rootVisual);
                    var deflateThickness = (Thickness)uiElement.GetValue(DirectionalNavigationMarginProperty);
            ...
            else
            {
                ...
                {
                    IContentHost? parentICH = null;
                    var parentUIElement = GetParentUIElementFromContentElement(ce, ref parentICH);
                    if (parentICH != null && parentICH is Visual parent && parentUIElement != null)
                    {
                        var rootVisual = GetVisualRoot(parent);
                        if (rootVisual != null)
                        {
                            ...
                            var rects = parentICH.GetRectangles(ce);
                ...
                {
                    if (element is UIElement3D uiElement3D)
                    {
                        var rootVisual = GetVisualRoot(uiElement3D);
                        var containingVisual2D = VisualTreeHelper.GetContainingVisual2D(uiElement3D);

                        if (rootVisual != null && containingVisual2D != null)
                        {
                            var rectElement = uiElement3D.Visual2DContentBounds;
                            var transform = containingVisual2D.TransformToAncestor(rootVisual);
        ...
        {
            var rect = GetRectangle(element);
            if (element is TreeViewItem tvi)
            {
                var itemsHost = tvi.ItemsHost;
                if (itemsHost != null && itemsHost.IsVisible)
                {
                    var itemsHostRect = GetRectangle(itemsHost);
                    if (itemsHostRect != Rect.Empty)
                    {
                        ...
                        var header = tvi.TryGetHeaderElement();
                        if (header != null && header != tvi && header.IsVisible)
                        {
                            var headerRect = GetRectangle(header);
                        ...
                        var before = itemsHostRect.Top - rect.Top;
                        var after = rect.Bottom - itemsHostRect.Bottom;
                        ...
                        {
                            var height = Math.Min(Math.Max(after, 0.0d), rect.Height);
        ...
        private double GetPerpDistance(Rect sourceRect, Rect targetRect, FocusNavigationDirection direction)
        {
            return direction switch
            {
                ...
                _ => throw new System.ComponentModel.InvalidEnumArgumentException("direction", (int)direction, typeof(FocusNavigationDirection)),
            };
        }
        ...
        {
            if (sourceElement is not Visual sourceVisual || targetElement is not Visual targetVisual)
            {
                return false;
            }
                ...
                case FocusNavigationDirection.Left:
                    ...
                    if (DoubleUtil.GreaterThan(targetRect.Bottom, startRange) && DoubleUtil.LessThan(targetRect.Top, endRange))
                    {
                        ...
                        return direction == FocusNavigationDirection.Right
                            ? DoubleUtil.GreaterThan(targetRect.Left, sourceRect.Left) || (DoubleUtil.AreClose(targetRect.Left, sourceRect.Left) && IsAncestorOfEx(sourceElement, targetElement))
                            : DoubleUtil.LessThan(targetRect.Right, sourceRect.Right) || (DoubleUtil.AreClose(targetRect.Right, sourceRect.Right) && IsAncestorOfEx(sourceElement, targetElement));
                    }
                    break;
                ...
                case FocusNavigationDirection.Down:
                    ...
                    if (DoubleUtil.GreaterThan(targetRect.Right, startRange) && DoubleUtil.LessThan(targetRect.Left, endRange))
                    {
                        ...
                        return direction == FocusNavigationDirection.Down
                            ? DoubleUtil.GreaterThan(targetRect.Top, sourceRect.Top) || (DoubleUtil.AreClose(targetRect.Top, sourceRect.Top) && IsAncestorOfEx(sourceElement, targetElement))
                            : DoubleUtil.LessThan(targetRect.Bottom, sourceRect.Bottom) || (DoubleUtil.AreClose(targetRect.Bottom, sourceRect.Bottom) && IsAncestorOfEx(sourceElement, targetElement));
                    }
                    break;
        ...
        private DependencyObject? GetNextInDirection(
            DependencyObject sourceElement,
            FocusNavigationDirection direction,
            bool treeViewNavigation,
            bool considerDescendants
        ...
        {
            _containerHashtable.Clear();
            var targetElement = MoveNext(sourceElement, null, direction, BASELINE_DEFAULT, BASELINE_DEFAULT, treeViewNavigation, considerDescendants);

            if (targetElement != null)
            {
                if (sourceElement is UIElement sourceUIElement)
                {
                    sourceUIElement.RemoveHandler(Keyboard.PreviewLostKeyboardFocusEvent, new KeyboardFocusChangedEventHandler(_LostFocus));
                }
                else
                {
                    var sourceContentElement = sourceElement as ContentElement;
                ...
                if (targetElement is not UIElement targetUIElement)
                {
                    targetUIElement = GetParentUIElementFromContentElement(targetElement as ContentElement);
                }
                else
                {
                    var targetContentElement = targetElement as ContentElement;
        ...
        {
            var elementObject = element ?? _fakeNull;

            // If entry exists then we have endless loop
            if (_containerHashtable[container] is Hashtable elementTable)
            {
            ...
            bool considerDescendants)   // when false, descendants of sourceElement are not candidates
        {
            ...
            {
                if (
                    currElement != sourceElement &&
                    IsGroupElementEligible(currElement, treeViewNavigation)
                )
                {
                    var currentRect = GetRepresentativeRectangle(currElement);
                    ...
                    if (currentRect != Rect.Empty)
                    {
                        ...
                        if (searchInsideContainer || isInDirection || isInRange)
                        {
                            ...
                            )
                            {
                                ...
                                (DoubleUtil.LessThan(score, resultScore) || (DoubleUtil.AreClose(score, resultScore) && GetDistance(sourceRect, resultRect, direction) > GetDistance(sourceRect, currentRect, direction)))
                                && (considerDescendants || !IsAncestorOfEx(sourceElement, currElement))
                            )
                            {
                                result = currElement;
        ...
        private DependencyObject? MoveNext(
            DependencyObject? sourceElement,
            DependencyObject? container,
            FocusNavigationDirection direction,
            double startRange,
        ...
        )
        {
            ...
            var sourceRect = searchInsideContainer ? GetRectangle(container)
            ...
            var result = FindNextInDirection(sourceElement, sourceRect, container, direction, startRange, endRange, treeViewNavigation, considerDescendants);
            ...
            if (result == null)
            {
                return mode switch
                {
                    ...
                    // Continue, Once, None, Local - search outside the container
            ...
            // Using ActiveElement if set
            var activeElement = GetActiveElementChain(result, treeViewNavigation);
            if (activeElement != null)
            {
                return activeElement;
            }
            ...
            // Try to find focus inside the element
            // result is not TabStop, which means it is a group
            var insideElement = MoveNext(null, result, direction, startRange, endRange, treeViewNavigation, considerDescendants: true);
        ...
        )
        {
            ...
            if (sourceElement != null)
            {
                ...
                var sourceElementPosition = ItemsControl.GetElementViewportPosition(
                    viewportBoundsElement,
            ...
            while ((currElement = GetNextInTree(currElement, container)) != null)
            {
                ...
                {
                    var currentRectElement = currElement;
                    if (treeViewNavigation)
                    {
        ...
        /// position ranges.
        /// </summary>
        private double ComputeRangeScore(
            double rangeStart1,
            double rangeEnd1,
            double rangeStart2,
            double rangeEnd2
        )
        {
            ...
            {
                (rangeStart2, rangeStart1) = (rangeStart1, rangeStart2);
                (rangeEnd2, rangeEnd1) = (rangeEnd1, rangeEnd2);
            }
        ...
        private void ProcessForMenuMode(InputEventArgs inputEventArgs)
        {
            ...
            {
                var args = inputEventArgs as KeyboardFocusChangedEventArgs;
            ...
            else if (inputEventArgs.RoutedEvent == Keyboard.KeyUpEvent)
            {
                ...
                {
                    var keyEventArgs = inputEventArgs as KeyEventArgs;
                    var realKey = GetRealKey(keyEventArgs);
            ...
            // The following input events act to "cancel" the EnterMenuMode event
                ...
                inputEventArgs.RoutedEvent == Mouse.MouseDownEvent ||
        ...
        private bool OnEnterMenuMode(object eventSource)
        {
            ...
            lock (_weakEnterMenuModeHandlers)
            {
                ...
                if (eventSource != null)
                {
                    source = (eventSource is Visual eventSourceVisual) ? PresentationSource.CriticalFromVisual(eventSourceVisual) : null;
                }
                else
                {
                ...
                _weakEnterMenuModeHandlers.Process(
                            delegate (object obj)
                            {
                                if (obj is EnterMenuModeEventHandler currentHandler)
                                {
        ...
        private void ProcessForUIState(InputEventArgs inputEventArgs)
        {
            ...
            var report = ExtractRawUIStateInputReport(inputEventArgs, InputManager.InputReportEvent);
        ...
        private RawUIStateInputReport ExtractRawUIStateInputReport(InputEventArgs e, RoutedEvent Event)
        {
            ...
            if (e is InputReportEventArgs input)
            {
        ...
        private WeakReferenceList _weakFocusEnterMainFocusScopeHandlers = new();
        #endregion
        ...
        private Hashtable _containerHashtable = new(10);
        private static object _fakeNull = new();
        ...
        private class WeakReferenceList : DispatcherObject
        {
            ...
            List<WeakReference> _list = new(1);
