...
namespace CleanWpfApp
{
    ...
    internal sealed class ContentType
    {
        ...
        internal ContentType(string contentType)
        {
            ...
            {
                if (IsLinearWhiteSpaceChar(contentType[0]) || IsLinearWhiteSpaceChar(contentType[^1]))
                    throw new ArgumentException(Strings.ContentTypeCannotHaveLeadingTrailingLWS);
        ...
        public override string ToString()
        {
            if (_contentType == null)
            {
                ...
                var stringBuilder = new StringBuilder(_type);
        ...
        {
            Debug.Assert(!IsLinearWhiteSpaceChar(contentType[0]) && !IsLinearWhiteSpaceChar(contentType[^1]));
            ...
            while (index != -1)
            {
                if (contentType[index - 1] == _LinearWhiteSpaceChars[1] || contentType[index + 1] == _LinearWhiteSpaceChars[1])
                {
                    ...
                    throw new ArgumentException(Strings.InvalidLinearWhiteSpaceCharacter);
            }
        }
        ...
        private void ParseTypeAndSubType(ReadOnlySpan<char> typeAndSubType)
        {
            ...
            if (forwardSlashPos < 0 || // no slashes
                typeAndSubType[(forwardSlashPos + 1)..].IndexOf('/') >= 0) // more than one slash
            {
                throw new ArgumentException(Strings.InvalidTypeSubType);
            }

            _type = ValidateToken(typeAndSubType[..forwardSlashPos].ToString());
            _subType = ValidateToken(typeAndSubType[(forwardSlashPos + 1)..].ToString());
        }
        ...
        private void ParseParameterAndValue(ReadOnlySpan<char> parameterAndValue)
        {
            while (!parameterAndValue.IsEmpty)
            {
                ...
                if (parameterAndValue[0] != _semicolonSeparator)
                    throw new ArgumentException(Strings.ExpectingSemicolon);
                ...
                if (parameterAndValue.Length == 1)
                    throw new ArgumentException(Strings.ExpectingParameterValuePairs);

                //Removing the leading ; from the string
                parameterAndValue = parameterAndValue[1..];
                ...
                if (equalSignIndex <= 0 || equalSignIndex == (parameterAndValue.Length - 1))
                    throw new ArgumentException(Strings.InvalidParameterValuePair);
                ...
                _parameterDictionary.Add(
                    ValidateToken(parameterAndValue[..equalSignIndex].ToString()),
                    ValidateQuotedStringOrToken(parameterAndValue.Slice(parameterStartIndex, parameterValueLength).ToString()));

                parameterAndValue = parameterAndValue[(parameterStartIndex + parameterValueLength)..].TrimStart(_LinearWhiteSpaceChars);
            }
        }
        ...
        private static int GetLengthOfParameterValue(ReadOnlySpan<char> s, int startIndex)
        {
            ...
            if (s[startIndex] != '"')
            {
                int semicolonIndex = s[startIndex..].IndexOf(_semicolonSeparator);

                if (semicolonIndex != -1)
                {
                    int lwsIndex = s[startIndex..].IndexOfAny(_LinearWhiteSpaceChars);
            ...
            else
            {
                ...
                while (!found)
                {
                    ...
                    length = s[startingLength..].IndexOf('"');

                    if (length == -1)
                        throw new ArgumentException(Strings.InvalidParameterValue);
        ...
        private static string ValidateToken(string token)
        {
            if (string.IsNullOrEmpty(token))
                throw new ArgumentException(Strings.InvalidToken);
        ...
        private static readonly ContentType _emptyContentType = new("");
        #endregion
    }
}
